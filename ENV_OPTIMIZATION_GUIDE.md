...
## Новые переменные для прибыльности (рекомендуется)

```env
# Режим стратегии: trend = следование за трендом (LONG на gainers, SHORT на losers)
# reversal = реверсивная (старое поведение). Для топ-муверов trend обычно лучше.
STRATEGY_MODE=trend

# Требовать ОБА условия (всплеск объема И momentum) для входа. 1=да (меньше сигналов, выше качество)
REQUIRE_BOTH_VOLUME_AND_MOMENTUM=1

# Минимальный score для принятия сигнала (60+ для качества)
